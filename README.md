...
The current source is **0.20.0-plane**, adding an optional shared-plane cover projection to the user-tested **0.18.1-entry** prototype. Its app grid stays on the right; local date and calendar cards appear on the left. Separate spatial blur masks soften the outgoing cover and reveal the incoming inner surface. User app selections persist. Cover touch is blocked while folding.

On Android 13+, Controls → **Perspektiv på utskärmen** enables cover-only projection through a rotating plane into the fixed inner image plane. It assumes a viewer centered over the closed cover, 400 mm away. The control starts disabled on fresh installs, preserves any existing selection, and leaves the inner scene unchanged. Disable it to compare with the previous blur-only effect. It replaces 0.19.0's heuristic transform, which the user found did not match the reference. This remains an experiment, not a measured reproduction of Apple's projection or a viewer-tracked transparent display. Content moves out of the cover crop while folding; rays beyond the source image repeat its edge pixels, which may show smearing. Closed geometry is unchanged.
