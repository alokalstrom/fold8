...
## 0.21.1 stable automatic folding

The user confirmed 0.21.0 broadly has the reference's visual ingredients and deferred fine calibration, but reported endpoint flashes. Logs showed `steadyTrial=false`, releasing the overlap at about 178° and 2°, while stored `steadyDisplay=false` and `automaticOverlap=true` were active. This matches the prior endpoint-remapping path. The exact cause of the changed preference/Home selection is not established.

Normal automation now always uses the already validated steady display policy. The independent legacy preference, old steadyTrial intent override and misleading “Avsluta försöket” control are no longer consumed/exposed. Automatic on/off and existing foreground lease, keyguard, stale-signal, navigation and background cleanup remain intact. No shader/projection/blur changes are made. The old preference can remain on disk but is ignored, permitting a real regression check without manually repairing settings. Device verification and user confirmation are tracked in issue #4 / PR #2; code alone does not prove that all possible display flashes are eliminated.
